...
pub fn evaluate_copy_permutation_monomials<'a, F: PrimeField>(
    worker: &Worker,
    monomial_storage: &CopyPermutationMonomials<F>,
    z: F,
    z_omega: F,
) -> CopyPermutationEvaluations<F> {
    let grand_product_at_z = monomial_storage
        .grand_product_monomial
        .evaluate_at(worker, z);
    let grand_product_at_z_omega = monomial_storage
        .grand_product_monomial
        .evaluate_at(worker, z_omega);

    let first_quotient_at_z = monomial_storage.first_quotient.evaluate_at(worker, z);
    let first_quotient_at_z_omega = monomial_storage.first_quotient.evaluate_at(worker, z_omega);
    let second_quotient_at_z = monomial_storage.second_quotient.evaluate_at(worker, z);
    let second_quotient_at_z_omega = monomial_storage
        .second_quotient
        .evaluate_at(worker, z_omega);
