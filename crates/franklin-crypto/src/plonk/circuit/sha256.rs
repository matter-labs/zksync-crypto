...
pub fn sha256_block_no_padding<E, CS>(cs: &mut CS, input: &[Boolean]) -> Result<Vec<Boolean>, SynthesisError>
where
    E: Engine,
    CS: ConstraintSystem<E>,
{
    ...
    Ok(sha256_compression_function(cs, &input, &get_sha256_iv())?.into_iter().flat_map(|e| e.into_bits_be()).collect())
}
...
pub fn sha256_compression_function<E, CS>(cs: &mut CS, input: &[Boolean], current_hash_value: &[UInt32]) -> Result<Vec<UInt32>, SynthesisError>
where
    E: Engine,
    CS: ConstraintSystem<E>,
{
    ...
    let mut w = input.chunks(32).map(|e| UInt32::from_bits_be(e)).collect::<Vec<_>>();
...
mod test {
    ...
    fn test_full_block() {
        ...
        let mut cs = TrivialAssembly::<Bn256, PlonkCsWidth4WithNextStepParams, Width4MainGateWithDNext>::new();
        let input_bits: Vec<_> = (0..512).map(|_i| Boolean::from(AllocatedBit::alloc(&mut cs, Some(rng.gen())).unwrap())).collect();
    ...
    fn test_against_vectors() {
        ...
        for input_len in (0..32).chain((32..256).filter(|a| a % 8 == 0)) {
            ...
            let mut s = hash_result.as_ref().iter().flat_map(|&byte| (0..8).rev().map(move |i| (byte >> i) & 1u8 == 1u8));
