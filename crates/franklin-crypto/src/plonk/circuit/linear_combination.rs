...
impl<E: Engine> LinearCombination<E> {
    ...
    pub fn into_allocated_num<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<AllocatedNum<E>, SynthesisError> {
    ...
    fn deduplicate_stable(terms: smallvec::SmallVec<[(E::Fr, Variable); DEFAULT_SMALLVEC_CAPACITY]>) -> smallvec::SmallVec<[(E::Fr, Variable); DEFAULT_SMALLVEC_CAPACITY]> {
    ...
    fn evaluate_term_value<CS: ConstraintSystem<E>>(cs: &CS, terms: &[(E::Fr, Variable)], constant: E::Fr) -> Result<E::Fr, SynthesisError> {
    ...
    fn enforce_zero_using_next_step<CS: ConstraintSystem<E>>(
        ...
        if num_terms <= CS::Params::STATE_WIDTH {
            ...
            let (mut next_step_var_in_chain, mut next_step_var_in_chain_value) = {
                ...
                let may_be_new_intermediate_value = Self::evaluate_term_value(&*cs, &chunk, constant_term);
            ...
            for _ in 0..(cycles - 1) {
                ...
                let may_be_new_intermediate_value = Self::evaluate_term_value(&*cs, &chunk, E::Fr::zero()).map(|val| {
                    // and
                    let mut tmp = val;
    ...
    pub fn unwrap_as_allocated_num(&self) -> Result<AllocatedNum<E>, SynthesisError> {
        ...
        if self.terms.len() == 1 && self.terms[0].0 == E::Fr::one() {
            ...
            let var = AllocatedNum { value: self.value, variable: var };
    ...
    pub fn uniquely_pack_le_booleans_into_single_num<CS: ConstraintSystem<E>>(cs: &mut CS, bools: &[Boolean]) -> Result<Num<E>, SynthesisError> {
        Self::uniquely_pack_booleans_into_single_num(cs, bools)
    }

    pub fn uniquely_pack_booleans_into_single_num<CS: ConstraintSystem<E>>(cs: &mut CS, bools: &[Boolean]) -> Result<Num<E>, SynthesisError> {
    ...
    pub fn uniquely_pack_booleans_into_nums<CS: ConstraintSystem<E>>(cs: &mut CS, bools: &[Boolean]) -> Result<Vec<Num<E>>, SynthesisError> {
        let mut result = vec![];
...
mod test {
    ...
    fn test_inscribe_linear_combination() {
        ...
        let variables: Vec<_> = (0..9).map(|_| AllocatedNum::alloc(&mut assembly, || Ok(Fr::one())).unwrap()).collect();
    ...
    fn test_inscribe_linear_combination_of_two_gates() {
        ...
        let variables: Vec<_> = (0..5).map(|_| AllocatedNum::alloc(&mut assembly, || Ok(Fr::one())).unwrap()).collect();
